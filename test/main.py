...
class profile(flask.views.MethodView):
	...
	def get(self):
		...
		return flask.render_template('profile.html',firstname=result[0][0],lastname=result[0][1],age=result[0][2],gender ="female" ,mail=result[0][4],country=result[0][5],education = result[0][6],work = result[0][7],college = result[0][8],school = result[0][9],biography=result[0][10],filename=result[0][11])
...
class tanzimprofile(flask.views.MethodView):
	...
	def get(self):
		user = flask.session['username']
		q="""match (p:People { username : "%s"}) return p.imageaddress""" % user
		result = gdb.query(q=q)
		print "wwwwwwwwwwwwwww= ",result[0][0]
		return flask.render_template('tanzimprofile.html',filename=result[0][0])
	...
	def post(self):
		...
		if 'save' in flask.request.form:
			if (y!="" and m!="" and da!=""):
				...
				q=""" create (r:Fdiaries{diarie:"%s"}) """ %(flask.request.form['text']) 
				result = gdb.query(q=q)
				...
				for i in range(len(result)):
					print result[i][0]
					q="""MATCH (p:People {username:"%s"}),(d:Fdiaries{diarie:"%s"})  CREATE (p)-[:fridiaries{year:"%s",month:"%s",day:"%s",titile:"%s"}]->(d) """ % (result[i][0],flask.request.form['text'],y,m,da,titl)
					results = gdb.query(q=q)
...
class friends(flask.views.MethodView):
	...
	def get(self):
		...
		for i in range(len(result)):
			x=dict([('firstname',result[i][0]),('lastname',result[i][1]),('image',result[i][2])])
			friends.append(x)
