...
app.config['UPLOAD_FOLDER'] = '/home/malihe/tinder/test/tinderajax/static/img/media'
...
!=""):
			...
			#	return flask.redirect(flask.url_for('signup'))
				...
				if os.path.exists('/home/malihe/tinder/test/tinderajax/static/img/media/'+str(flask.request.form['username1'])):
    					pass
				else:
    					os.mkdir('/home/malihe/tinder/test/tinderajax/static/img/media/'+str(flask.request.form['username1']))

			
				if os.path.exists('/home/malihe/tinder/test/tinderajax/static/img/media/'+str(flask.request.form['username1'])+str("khatere")):
    					pass
				else:
    					os.mkdir('/home/malihe/tinder/test/tinderajax/static/img/media/'+str(flask.request.form['username1'])+str("khatere"))
...
class home(flask.views.MethodView):
	...
	def get(self):
		...
		for i in range(len(result)):
			...
			q=""" match (d:Diaries{ID:"%s"})-[r:imagerel]->(i:Image) return i.address""" #ax haie ke ba khatere ferestade
			img = gdb.query(q=q)
			...
			x = dict([('text',result[i][0]),('user',result[i][1]),('userimage',picture[0][0]),('year',result[i][2]) , ('month',result[i][3]) , ('day',result[i][4]) , ('title',result[i][5]) , ('feel',"laugh") , ('comment',comen),('image',imag),('like',like[0][0]),('id',result[i][6])])
			dfriends.append(x)
		...
		if 'save' in flask.request.form:
			...
			if (y!="" and m!="" and da!=""):
				if flask.request.form['privacy']=="private":
					...
					q = """ match (g:ID) set g.count="%s" """%(int(count[0][0])+1)
					result = gdb.query(q=q)
...
class diary1(flask.views.MethodView):
	...
	def get(self):
		...
		for i in range(len(result)):
			if (result[i][3]==str(1)):
				x=dict([('text',result[i][0]),('user',result[i][1]),('year',result[i][2]) , ('month',result[i][3]) , ('day',result[i][4]) , ('title',result[i][5])])
				diary.append(x)
...
class public_user(flask.views.MethodView):
	...
	def get(self):
		...
		for i in range(len(result)):
			...
			imm = []
			publics=[]
			x=dict([('text',result[i][0]),('user',result[i][1]),('year',result[i][2]) , ('month',result[i][3]) , ('day',result[i][4]) , ('title',result[i][5]),('image',img[0][0]),('images',imm)])
